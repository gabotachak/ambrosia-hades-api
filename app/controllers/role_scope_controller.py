...
class RoleScopeController:
    ...
    def _set_scopes_in_role(self, role: RoleEntity, scopes_req):
        """Set scopes to a specified role"""

        self.db_session.query(RoleScopeEntity).filter(RoleScopeEntity.role_id == role.role_id).delete()

        return self._add_scopes_to_role(role, scopes_req)

    def set_scopes_in_role_by_id(self, role_id: str, scopes_req):
        """Set scopes to a specified role by id"""

        role = self.db_session.query(RoleEntity).filter_by(role_id=role_id).first()
        if not role:
            raise RoleNotFoundException(role_id)

        return self._set_scopes_in_role(role, scopes_req)

    def set_scopes_in_role_by_name(self, role_name: str, scopes_req):
        """Set scopes to a specified role by name"""
